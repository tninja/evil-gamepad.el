...
core navigation and editing instead of third-party integrations. The =j=
binding can optionally use =avy= when that package is installed.
...
If you want the =j= binding to jump with =avy=, install =avy= separately.
Without =avy=, loading =evil-gamepad= still succeeds and the binding will show
